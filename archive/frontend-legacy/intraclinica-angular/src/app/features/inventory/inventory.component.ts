    ...
    <div class="space-y-6 animate-fade-in relative">
      @if (!db.checkPermission('inventory.read', db.selectedContextClinic() || db.currentUser()?.clinicId)) {
        <div class="flex flex-col items-center justify-center min-h-[60vh] text-center p-8 bg-white rounded-[40px] border border-slate-200 shadow-sm animate-scale-in">
            ...
            <p class="text-slate-500 max-w-md mx-auto mb-8 font-medium">Você não possui permissão para visualizar o estoque desta clínica. Caso seja um Super Admin ou queira solicitar acesso temporário, clique no botão abaixo.</p>
            <button (click)="handleRequestClinicAccess()" class="px-8 py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-slate-800 transition-all shadow-xl shadow-slate-200 flex items-center gap-2">
                <lucide-icon [img]="ShieldAlert" [size]="16"></lucide-icon> Solicitar Acesso Temporário
            </button>
        </div>
      } @else {
        <!-- SCANNER OVERLAY -->
...
export class InventoryComponent {
  ...
  async handleDeleteProduct(p: Product) {
      ...
      if (confirm(`Tem certeza que deseja excluir "${p.name}"? Esta ação não pode ser desfeita.`)) {
          await this.db.deleteProduct(p.id);
      }
  }
  ...
  handleScanSubmit(event: Event) {
    ...
    const product = this.db.products().find(p => p.id === code);
    ...
    if (product) {
        ...
        // Aprimorado: Se não encontrar, abre o cadastro com o código já preenchido
        setTimeout(() => {
            this.scannerMode.set(false);
            this.openNewProductForm();
            this.formData.id = code; // Preenche o ID com o código lido
        }, 800);
    }
  ...
  async handleAiAnalysis() {
    ...
    const result = await this.gemini.analyzeInventoryRisks(this.db.products());
  ...
  async handleFileImport(event: Event) {
    ...
    // Auto-process unique new items, flag conflicts for others
    const conflicts = this.csvService.detectConflicts(parsedProducts, this.db.products());
    
    // Process items with no conflicts immediately
    ...
    for (const item of cleanItems) {
      if (item.id && item.name) {
        await this.db.addProduct(item as Product);
      }
    }
  ...
  async handleConflictResolution(event: { conflict: any, product: Product, action: 'CREATE' | 'UPDATE' | 'KEEP_EXISTING' }) {
    ...
    if (action === 'CREATE') {
      if (conflict.type === 'NEW') {
          await this.db.addProduct(product as Product);
      }
      ...
      await this.db.addProduct(merged);
    }
  ...
  exportCsv() {
    ...
    const csvContent = [
      ...
      ...products.map(p => [
        p.id,
        `"${p.name}"`, // Enclosed in double quotes
        `"${p.category}"`, // Enclosed in double quotes
        p.stock,
        p.minStock,
        `"${p.price.toFixed(2).replace('.', ',')}"`, // Enclosed in double quotes
        `"${(p.costPrice || 0).toFixed(2).replace('.', ',')}"`, // Enclosed in double quotes
        p.expiryDate ? new Date(p.expiryDate).toLocaleDateString('pt-BR') : '',
        `"${p.supplier}"`, // Enclosed in double quotes
        `"${p.notes || ''}"` // Enclosed in double quotes
      ].join(','))
