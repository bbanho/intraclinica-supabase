---
title: "Architecture Audit (Mar 2026)"
description: "Complete codebase audit of IntraClinica - database schema, core services, and component analysis"
...
* **ReceptionComponent**: Real-time queue, status buttons (Scheduled -> Waiting). Uses incorrect IAM (`reception.read`).
* **AppointmentsListComponent**: EMPTY STUB.
* **PatientsListComponent**: Lists patients, but search doesn't work.
* **ClinicalComponent**: AI Medical Record. Reads from `patientStore.appointments` and `patientStore.records`. "Recent History" shows all clinic records, doesn't filter by patient.
* **ClinicalExecutionComponent**: Executes procedures. Not reactive to patient store.
* **InventoryComponent**: Stock. Double source of truth problem (local signal vs InventoryStore). Broken barcode.
* **ReportsComponent**: Dashboard. `todayAppointmentsCount` counts everything, doesn't filter by today.
* **SocialComponent**: AI social posts. No real image generation and no network selector.
* **AdminPanelComponent**: SaaS settings and IAM role-based control (Role based).

## REFACTORING ROADMAP (Next Steps)

1. Consolidate `AppointmentsListComponent` into `ReceptionComponent` using a TAB system (Queue / Weekly Schedule).
2. Ensure the Schedule can create appointments that feed the queue (status 'Waiting').
3. Refactor `ClinicalComponent` to load correct patient data for the active appointment.
