...
CREATE POLICY "clinic_module is viewable by clinic members" ON clinic_module FOR SELECT USING (clinic_id IN (SELECT auth.clinic_id()));
CREATE POLICY "clinic_config is viewable by clinic members" ON clinic_config FOR SELECT USING (clinic_id IN (SELECT auth.clinic_id()));
