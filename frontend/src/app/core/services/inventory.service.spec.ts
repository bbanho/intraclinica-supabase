...
function createMockSupabaseClient() {
  ...
  const mockRpc = vi.fn();
  const mockClient = { from: mockFrom, rpc: mockRpc } as any;
...
describe('InventoryService', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      providers: [
        InventoryService,
        { provide: SupabaseService, useValue: { clientInstance: mockSupabaseClient.mockClient } },
        { provide: ClinicContextService, useValue: { selectedClinicId: signal(MOCK_CLINIC_ID) } },
      ],
    }).compileComponents();
  ...
  describe('getProducts()', () => {
    it('should return products filtered by clinic_id ordered by name', async () => {
      ...
      expect(result[0].name).toBe('Esparadrapo');
    });

    it('should map avg_cost_price DB column to cost property', async () => {
      const dbRow = { id: 'prod-x', clinic_id: MOCK_CLINIC_ID, name: 'Test Product', category: 'Test', price: 10, avg_cost_price: 4.5, min_stock: 5, current_stock: 10, barcode: null };
      const mockQuery = {
    ...
    it('should throw when clinic context is "all" (Super Admin global view)', async () => {
      await TestBed.configureTestingModule({
        providers: [
          InventoryService,
          { provide: SupabaseService, useValue: { clientInstance: mockSupabaseClient.mockClient } },
          { provide: ClinicContextService, useValue: { selectedClinicId: signal('all') } },
        ],
      }).compileComponents();

      const svc = TestBed.inject(InventoryService);
      await expect(svc.getProducts()).rejects.toThrow('Invalid or missing Clinic Context for Inventory feature.');
    });
    ...
    it('should throw when clinic context is null (no clinic selected)', async () => {
      await TestBed.configureTestingModule({
        providers: [
          InventoryService,
          { provide: SupabaseService, useValue: { clientInstance: mockSupabaseClient.mockClient } },
          { provide: ClinicContextService, useValue: { selectedClinicId: signal(null) } },
        ],
      }).compileComponents();
  ...
  describe('createProduct()', () => {
    const mockDto: CreateProductDto = {
      name: 'Gaze Estéril', category: 'Curativos', price: 12.0, cost: 6.0, min_stock: 10, current_stock: 50, barcode: '9998887776665',
    };
    ...
    it('should call RPC create_product_with_stock with correct parameters', async () => {
      ...
      expect(mockSupabaseClient.mockRpc).toHaveBeenCalledWith('create_product_with_stock', {
        p_clinic_id: MOCK_CLINIC_ID, p_name: mockDto.name, p_category: mockDto.category,
        p_price: mockDto.price, p_cost: mockDto.cost, p_min_stock: mockDto.min_stock,
        p_current_stock: mockDto.current_stock, p_barcode: mockDto.barcode,
      });
    ...
    it('should default cost to 0 when falsy', async () => {
      ...
      expect(mockSupabaseClient.mockRpc).toHaveBeenCalledWith('create_product_with_stock', expect.objectContaining({ p_cost: 0 }));
    });

    it('should default min_stock and current_stock to 0 when falsy', async () => {
      const dtoWithDefaults: CreateProductDto = { ...mockDto, min_stock: 0 as any, current_stock: 0 as any };
      mockSupabaseClient.mockRpc.mockResolvedValue({ data: null, error: null });
    ...
    it('should throw when RPC returns an error', async () => {
      mockSupabaseClient.mockRpc.mockResolvedValue({ data: null, error: { message: 'RPC failed' } });
